...
def repfb_xcorr_avg(idxs,files,pfb_size,nchunks,chanstart,chanend,osamp,cutsize=16,filt_thresh=0.45):
    ...
    for i, chunks in enumerate(zip(*antenna_objs)):
        ...
        for j in range(nant):
            ...
            print(f"\tIPFB AVG time {np.mean(tot[0])} +- {np.std(tot[0])} s, BW {to_ipfb_pol0.nbytes/np.mean(tot[0])/1e6} MSPS\n") 
            print(f"\tPFB AVG time {np.mean(tot[1])} +- {np.std(tot[1])} s, BW {raw_pol0[cut:-cut].nbytes/np.mean(tot[1])/1e6} MSPS\n")
            ...
            # print("in shape",pol0_new[:, repfb_chanstart : repfb_chanend].shape )
