...
def repfb_xcorr_avg(idxs,files,pfb_size,nchunks,chanstart,chanend,osamp,cutsize=16,filt_thresh=0.45, n_cores=None):
    ...
    for i, chunks in enumerate(zip(*antenna_objs)):
        ...
        for j in range(nant):
            ...
            pu.cpu_ipfb(to_ipfb_pol0, matft, raw_pol0, inv_matft, thresh=filt_thresh, cut=cut)
            ...
            # from albatros_analysis.src.utils import test_speed_cpu as speed
            print(f"Starting Pipeline speed test with {niter} Iterations...")
            ...
            for z in range(niter):
                t0 = time.time()
                pu.cpu_ipfb(to_ipfb_pol0, matft, raw_pol0, inv_matft, thresh=filt_thresh, cut=cut)
                tot[0].append(time.time()-t0)
                tot[2].append(time.time()-t0)
                if z%100==0:
                    print(z)

            pu.cpu_pfb(raw_pol0,cpu_win_big, out=pol0_new, scratch=pfb_scratch, nchan=2048*osamp+1,ntap=4)
           
            for z in range(niter):
                t1 = time.time()
                pu.cpu_pfb(raw_pol0,cpu_win_big, out=pol0_new, scratch=pfb_scratch, nchan=2048*osamp+1,ntap=4)

                tot[1].append(time.time()-t1)
                tot[2][z] += time.time()-t1
            ...
            print(f"Pipeline AVG time {np.mean(tot[2])} +- {np.std(tot[2])} s, BW {to_ipfb_pol0.nbytes/np.mean(tot[2])/1e6} MSPS\n")

            print(f"\tFFTW IPFB AVG time {np.mean(tot[0])} +- {np.std(tot[0])} s, BW {to_ipfb_pol0.nbytes/np.mean(tot[0])/1e6} MSPS\n") 
            print(f"\tFFTW PFB AVG time {np.mean(tot[1])} +- {np.std(tot[1])} s, BW {raw_pol0[cut:-cut].nbytes/np.mean(tot[1])/1e6} MSPS\n")



            from albatros_analysis.src.utils import pfb_cpu_utils_scipy as spu
            print(f"Starting Pipeline speed test with {niter} Iterations...")
            print("SciPy Pipeline")
            tot = [[], [], []]
            
            
            spu.cpu_ipfb(to_ipfb_pol0, matft, raw_pol0, inv_matft, thresh=filt_thresh, cut=cut)
            for z in range(niter):
                t0 = time.time()
                spu.cpu_ipfb(to_ipfb_pol0, matft, raw_pol0, inv_matft, thresh=filt_thresh, cut=cut)
                tot[0].append(time.time()-t0)
            ...
            spu.cpu_pfb(raw_pol0,cpu_win_big, out=pol0_new, scratch=pfb_scratch, nchan=2048*osamp+1,ntap=4)
            ...
            #     t0 = time.time()
            #     raw_pol0 = speed.cpu_ipfb2(to_ipfb_pol0, matft, inv_matft, thresh=filt_thresh)
            #     pol0_new = speed.cpu_pfb2(raw_pol0[cut:-cut],cpu_win_big.ravel(),nchan=2048*osamp+1,ntap=4)
            #     tot += time.time()-t0
            # avg_time = tot/niter
            # print(f"AVG time {avg_time} s, BW {to_ipfb_pol0.nbytes/avg_time/1e6} MSPS\n")

            raise ValueError("I'm Done here")

            raw_pol1 = pu.cpu_ipfb(to_ipfb_pol1, matft, inv_matft, thresh=filt_thresh)
            pol1_new = pu.cpu_pfb(raw_pol1[cut:-cut],cpu_win_big,nchan=2048*osamp+1,ntap=4)

            # with timer("final exports"):
