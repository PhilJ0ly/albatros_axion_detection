...
fn = "/lib_correlations_cpu.so"
if xp.__name__=='cupy':
    from .correlations_gpu import *
    fn = "/lib_correlations_cpu2.so"

mylib = ctypes.cdll.LoadLibrary(
    os.path.realpath(__file__ + r"/..") + fn
)
...
@njit(parallel=True)
def avg_xcorr_all_ant_cpu(x, nant, npol, ntime, nfreq, split=1, out=None):
    """
    ...
    if out is None:
        out = xp.empty((M, M, nfreq), dtype=np.complex64)
    ...
    for f in prange(nfreq):
        ...
        for i in range(M):
            for j in range(i, M):  # Only compute upper triangle + diagonal
                ...
                if i != j:
                    out[j, i, f] = xp.conj(result)
    
    return out

# Philippe Joly 2025-06-05
def avg_xcorr_all_ant_cpu_og(x: xp.ndarray, nant: int, npol: int, ntime: int, nfreq: int, split: int = 1, scratch=None, out=None):
    M = nant * npol
    N = M
    K = ntime
    ...
    if out is None:
        out = xp.empty((M, N, nfreq), dtype='complex64', order='C')
